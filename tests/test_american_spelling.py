...
Ported from haversack (commit 514332c) on 2026-09-22, the day the house spelling was made
critical across the user's repositories. The reason is DRIFT: every British word already in a
file is a template the next edit copies - into this repository and, through feldglas and
haversack, out of it - so a clean tree has to be held clean by a test, not by care. Tests and
tools are scanned too - they are not user facing, but they are where the copying starts.
...
- There is no repository-wide exception. provender is published and its public names and the keys
  it writes into stored objects are never renamed, so if one of them ever has to keep a British
  spelling it says so with the pragma below, on its line.
...
_OTHER = {
    ...
    # the medical forms the consuming repositories meet, kept so the lists stay one list
...
class TestTheScanSeesTheRepo(unittest.TestCase):
    ...
    def test_the_scan_covers_the_package_the_docs_and_the_tests(self):
        ...
        for must in ("src/provender/__init__.py", "src/provender/blobs.py", "src/provender/store.py",
                     "README.md", "pyproject.toml", "tests/test_blobs.py", "tools/probe_store.py"):
...
class TestTheDetector(unittest.TestCase):
    def test_catches_prose_and_identifiers(self):
        for line, word in (("the centre of the box", "centre"), ("colourMap = cm.viridis", "colour"),
                           ...
                           ("the put was cancelled", "cancelled"), ("serialised bytes", "serialised"),
                           ("Behaviour under load", "behaviour")):
